...
문제
알파벳 대문자로 구성되어있는 문자열 S가 주어졌을 때, S에 등장하지 않는
알파벳 대문자의 아스키 코드 값의 합을 구하는 프로그램을 작성하시오.
...
for i in range(T):
    alphabet = [k for k in range(65, 91)]
    string = input()
    for j in range(len(string)):
        if ord(string[j]) in alphabet:
            alphabet.remove(ord(string[j]))
    print(sum(alphabet))
